...
int Digital::setupDigitalInterrupt(int interruptMode){
	int i=_setupDigitalInterrupt(interruptMode);
	return i;
};

/*int Digital::setupDigitalInPullUpInterrupt(int interruptMode){
	int i=_setupDigitalInPullUpInterrupt(interruptMode);
	if(i)return i;
	_digitalMode=DIGITAL_MODE_PULLUP;
	return 0;
}

int Digital::setupDigitalInPullDownInterrupt(int interruptMode){
	int i=_setupDigitalInPullDownInterrupt(interruptMode);
	if(i)return i;
	_digitalMode=DIGITAL_MODE_PULLDOWN;
	return 0;
}*/
